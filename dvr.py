        ...
        for i in range(dim_basis):
            for ip in range(dim_basis):
                ...
                if i == ip:
                    ...
                    tmp[i, ip] = 2 * (-1)**(i_idx - ip_idx)/(np.pi**2*dim_basis**2*(i_idx - ip_idx)**2)
                tmp[i, ip] *= np.pi**2 / (L**2)
        tmp = tmp / (2. * basis_specs[1][3])
    ...
    if basis_specs[0] == 'EXPO':
        ...
        for i in range(dim_basis):
            for ip in range(dim_basis):
                ...
                ip_idx = ip + 1 
                if i == ip:
                    tmp[i, i] = ((dim_basis**2 - 1) / 3.)
                else:
                    tmp[i, ip] = 2*((-1)**(i_idx - ip_idx))*(np.cos(np.pi / (dim_basis) *(i_idx - ip_idx))/
                        (np.sin(np.pi / (dim_basis) *(i_idx - ip_idx))**2))
                tmp[i, ip] *= np.pi**2 / (L**2)
